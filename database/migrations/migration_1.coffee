...
exec = () ->
    ...
    db.mysql.schema.hasTable('auth_user').then (exists)->
      if exists
        db.mysql.schema.renameTable('auth_user', 'user').then ()->
          console.log ' Table auth_user renamed to user successfully.'
      else
        console.log ' Table auth_user does not exist.'

  done = done.then () ->
    db.mysql.schema.hasTable('entry_entry').then (exists)->
      if exists
        db.mysql.schema.renameTable('entry_entry', 'entry').then ()->
          console.log ' Table entry_entry renamed to entry successfully.'
      else
        console.log ' Table entry_entry does not exist.'

  done = done.then () ->
    db.mysql.schema.hasTable('announcement_announcement').then (exists)->
      if exists
        db.mysql.schema.renameTable('announcement_announcement', 'announcement').then ()->
    ...
    db.mysql.schema.hasTable('social_auth_usersocialauth').then (exists)->
      if exists
        db.mysql.schema.renameTable('social_auth_usersocialauth', 'user_social').then ()->
    ...
    db.mysql.schema.hasTable('announcement_announcementconfirmation').then (exists)->
      if exists
        db.mysql.schema.renameTable('announcement_announcementconfirmation', 'announcement_confirmation').then ()->
    ...
    db.mysql.schema.hasTable('entry_entrycomment').then (exists)->
      if exists
        db.mysql.schema.renameTable('entry_entrycomment', 'entry_comment').then ()->
    ...
    db.mysql.schema.hasTable('user_userclient').then (exists)->
      if exists
        db.mysql.schema.renameTable('user_userclient', 'user_client').then ()->
          console.log ' Table user_userclient renamed to user_client successfully.'
      else
        console.log ' Table user_userclient does not exist.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('auth_group').then ()->
      console.log ' Table auth_group droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('auth_group_permissions').then ()->
      console.log ' Table auth_group_permissions droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('auth_permission').then ()->
      console.log ' Table auth_permission droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('auth_user_groups').then ()->
      console.log ' Table auth_user_groups droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('auth_user_user_permissions').then ()->
      console.log ' Table auth_user_user_permissions droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('django_admin_log').then ()->
      console.log ' Table django_admin_log droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('django_content_type').then ()->
      console.log ' Table django_content_type droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('django_session').then ()->
      console.log ' Table django_session droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('django_site').then ()->
      console.log ' Table django_site droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('social_auth_association').then ()->
      console.log ' Table social_auth_association droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('social_auth_nonce').then ()->
      console.log ' Table social_auth_nonce droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('south_migrationhistory').then ()->
      console.log ' Table south_migrationhistory droped successfully.'

  done = done.then () ->
    db.mysql.schema.dropTableIfExists('user_userprofile').then ()->
